...
const statusMeta: Record<string, { label: string; cls: string; tone: 'blue' | 'green' | 'red' | 'yellow' }> = {
  LOGISTICS_PICKUP_DONE:      { label: 'Em trânsito',         cls: 'bg-primary/10 text-primary border border-primary/20',        tone: 'blue' },
  LOGISTICS_DELIVERY_PENDING: { label: 'Saiu p/ entrega',     cls: 'bg-primary/10 text-primary border border-primary/20',        tone: 'blue' },
  ...
  LOGISTICS_REQUEST_CREATED:  { label: 'Coleta solicitada',   cls: 'bg-tertiary/10 text-tertiary border border-tertiary/20',     tone: 'yellow' },
  LOGISTICS_PICKUP_REQUESTED: { label: 'Coleta agendada',     cls: 'bg-tertiary/10 text-tertiary border border-tertiary/20',     tone: 'yellow' },
  LOGISTICS_READY:            { label: 'Pronto p/ retirada',  cls: 'bg-tertiary/10 text-tertiary border border-tertiary/20',     tone: 'yellow' },
  LOGISTICS_INVOICE_PENDING:  { label: 'NF pendente',         cls: 'bg-tertiary/15 text-tertiary border border-tertiary/30',     tone: 'yellow' },
}
...
function statusClass(status: string | null): string {
  if (!status) return 'bg-outline/15 text-outline border border-outline/25'
  return statusMeta[status]?.cls ?? 'bg-outline/15 text-outline border border-outline/25'
}
...
const categoryColor: Record<Category, { text: string; ring: string; bg: string; border: string }> = {
  in_transit: { text: 'text-primary',   ring: 'ring-primary/40',   bg: 'bg-primary/10',   border: 'border-primary/50' },
  ...
  pending:    { text: 'text-tertiary',  ring: 'ring-tertiary/40',  bg: 'bg-tertiary/10',  border: 'border-tertiary/50' },
}
...
function SummaryCard({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        'glass-card flex flex-col gap-sm rounded-xl p-lg text-left transition-all active:scale-[0.98]',
...
function ShipmentDrawer({ shipment, onClose }: { shipment: ShipmentRow; onClose: () => void }) {
  ...
  return (
    <aside className="absolute bottom-margin right-margin top-margin z-20 flex h-[calc(100vh-56px-80px)] w-[380px] flex-col overflow-hidden rounded-xl border-l border-white/10 bg-[#0d1117]/85 backdrop-blur-[20px]">
      <div className="flex items-start justify-between border-b border-white/10 p-lg">
        <div className="min-w-0 pr-2">
          <h3 className="text-base font-semibold text-on-background">Detalhes do Envio</h3>
          <p className="mt-1 truncate font-mono text-xs text-outline">#{shipment.order_sn}</p>
        </div>
        <button
          type="button"
      ...
      <div className="flex flex-1 flex-col gap-xl overflow-y-auto p-lg">
        <div className="flex flex-col gap-xs">
          ...
          <div className="flex items-center justify-between rounded-lg border border-white/10 bg-[#050507] p-sm">
            {shipment.tracking_number ? (
              <span className="font-mono text-base tracking-widest text-tertiary">
                {shipment.tracking_number}
              </span>
            ) : (
              <span className="text-sm text-outline">Tracking não disponível</span>
            )}
            <button
              type="button"
              ...
              className={cn(
                'flex items-center gap-1 rounded p-1 text-outline transition-colors hover:text-white',
          ...
          {shipment.shipping_carrier && (
            <span className="mt-1 text-xs text-outline">via {shipment.shipping_carrier}</span>
          )}
        </div>

        <div className="flex flex-col gap-md">
          <span className="text-xs font-medium uppercase text-on-surface-variant">Status atual</span>
        ...
        <div className="flex flex-col gap-md">
          <span className="text-xs font-medium uppercase text-on-surface-variant">Histórico</span>
          {loading ? (
            <div className="flex flex-col items-center gap-2 rounded-lg border border-white/10 bg-white/5 p-md text-center">
              <Icon name="progress_activity" className="animate-spin text-tertiary" size={20} />
              <span className="text-xs text-outline">Carregando histórico…</span>
            </div>
          ) : !history || history.length === 0 ? (
            <div className="rounded-lg border border-white/10 bg-white/5 p-md text-center text-xs text-outline">
                ...
                return (
                  <div key={ev.id} className="relative mb-lg flex gap-md">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 font-mono text-[11px] text-on-surface-variant">{fmtDateTime(ev.event_date)}</p>
                    </div>
                  </div>
                )
              })}
        ...
        <div className="mt-auto flex flex-col gap-xs">
          <span className="text-xs font-medium uppercase text-on-surface-variant">Pedido vinculado</span>
          <div className="rounded-lg border border-white/5 bg-white/5 p-md">
            <p className="font-mono text-xs text-outline">#{shipment.order_sn}</p>
            {shipment.receiver_name && (
              <p className="mt-1 text-sm font-semibold text-on-background">{shipment.receiver_name}</p>
            )}
            <p className="text-sm leading-relaxed text-on-surface-variant">{destination}</p>
            <div className="mt-2 flex items-center justify-between border-t border-white/5 pt-2 text-xs text-outline">
              <span>{buyer ?? '—'}</span>
              {orderDate && <span className="text-on-surface">{fmtDate(orderDate)}</span>}
              {orderTotal != null && <span className="text-on-surface">{fmtBrl(orderTotal)}</span>}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-white/10 p-md">
        <button className="flex w-full items-center justify-center gap-xs rounded-lg bg-tertiary py-sm text-base font-semibold text-on-tertiary transition-colors hover:bg-tertiary/90">
      ...
      <main className={cn('relative flex flex-1 overflow-y-auto p-margin', pending && 'opacity-70 transition-opacity')}>
        <div className={cn('mx-auto flex w-full flex-1 flex-col gap-gutter', selected ? 'max-w-[calc(100%-400px)] pr-gutter' : 'max-w-7xl')}>
          <div>
            ...
            <p className="text-sm text-on-surface-variant">Rastreie e gerencie envios da loja Shopee.</p>
          </div>
          ...
          <div className="glass-card flex flex-wrap items-center justify-between gap-md rounded-xl p-md">
            <div className="flex flex-wrap items-center gap-md">
              <div className="relative">
                ...
                <input
                  ...
                  className="h-10 w-64 rounded-lg border border-white/10 bg-[#050507] pl-xl pr-md text-sm text-on-background outline-none transition-all focus:border-tertiary focus:ring-1 focus:ring-tertiary"
              ...
              <select
                ...
                className="h-10 appearance-none rounded-lg border border-white/10 bg-[#050507] px-md py-sm pr-xl text-sm text-on-background outline-none transition-all focus:ring-1 focus:ring-tertiary"
              >
              ...
              <div className="flex h-10 rounded-lg border border-white/10 bg-[#050507] p-1">
            ...
            <div className="flex items-center gap-md">
              <span className="text-xs text-on-surface-variant">{totalCount} envios</span>
            </div>
          </div>
          ...
          <div className="glass-card flex flex-1 flex-col overflow-hidden rounded-xl">
            {shipments.length === 0 ? (
              <div className="flex flex-col items-center gap-2 p-xl text-center">
                <Icon name="local_shipping" className="text-3xl text-outline-variant" />
                  ...
                  <tbody>
                    {shipments.map((s) => {
                      ...
                      return (
                        ...
                        >
                          <td className={cn('px-lg py-md font-mono text-xs', isProblem ? 'text-error' : 'text-tertiary')}>
                            {s.tracking_number ?? <span className="text-outline-variant">—</span>}
                          </td>
                          <td className="px-lg py-md font-mono text-xs text-on-surface-variant">{s.order_sn}</td>
                          <td className="px-lg py-md font-mono text-xs text-on-background">{s.shopee_orders?.buyer_username ?? '—'}</td>
                          <td className="px-lg py-md text-on-surface-variant">{dest}</td>
                          ...
                          <td className="px-lg py-md text-xs text-on-surface-variant">{fmtRelative(s.synced_at)}</td>
                          <td className="px-lg py-md text-xs text-on-surface-variant">{orderDate ? fmtDate(orderDate) : '—'}</td>
                        </tr>
                      )
                    })}
                  </tbody>
